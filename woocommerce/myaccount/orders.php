					...
					?>
						<?php if ($is_order_number) : ?>
							...
							<?php elseif ($is_order_number) : ?>
								...
								<div class="order-thumbnail-wrapper" style="display:flex; flex-wrap:wrap; gap:10px; margin-top:10px;">
									<?php foreach ($order->get_items() as $item_id => $item) :
										$product = $item->get_product();
										if ($product) : ?>
											<div style="width:60px; text-align:center; font-size:11px;">
												<div style="width:100%;">
...
<?php else : ?>
	...
	<?php wc_print_notice(esc_html__('No order has been made yet.', 'woocommerce') . ' <a class="submit__btn' . esc_attr($wp_button_class) . '" href="' . esc_url(apply_filters('woocommerce_return_to_shop_redirect', wc_get_page_permalink('shop'))) . '">' . esc_html__('Browse products', 'woocommerce') . '</a>', 'notice'); // phpcs:ignore WooCommerce.Commenting.CommentHooks.MissingHookComment 
	?>
