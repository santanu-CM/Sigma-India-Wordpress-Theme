...
/**
 * Template Name: Shooting With Sigma
 */
get_header();
?>
...
<div class="body__container__inner">
    <div class="heading__wrap heading__wrap__medium large__block container__fluid__wrap">
        <div class="content__width560__pc">
            ...
            <p class="text__center spacing__40"><?php echo get_field('sub_title'); ?></p>
        </div>
    </div>
    ...
    <div class="large__block container__fluid__wrap">
        ...
        $args = array(
            'post_type' => 'shooting_with_sigma',
            'posts_per_page' => 12,
            'paged' => $paged,
            'post_status' => 'publish',
        );

        $sigma_query = new WP_Query($args);
        ?>

        <!-- Gallery Grid -->
        <div id="gallery" class="l__panel__grid medium__block panel__3__pc">
            <?php
            if ($sigma_query->have_posts()):
                while ($sigma_query->have_posts()): $sigma_query->the_post();

                    $terms = get_the_terms(get_the_ID(), 'shooting_category');
                    $term_classes = '';
                    $term_name = '';
            ...
            ?>
                <div class="content__card gallery__item mix<?php echo $term_classes; ?>" style="display: block;">
                    <a href="<?php the_permalink(); ?>">
                        ...
                        <?php if (get_field('shot_on')): ?>
                            <p class="f__uppercase spacing__16"><?php echo get_field('shot_on'); ?></p>
                        <?php endif; ?>
                    </a>
                </div>
            <?php
        ...
        <!-- Pagination -->
        <?php
        $total_pages = $sigma_query->max_num_pages;
        if ($total_pages > 1): ?>
